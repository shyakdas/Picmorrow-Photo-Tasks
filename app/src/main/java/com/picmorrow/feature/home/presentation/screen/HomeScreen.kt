...
import com.picmorrow.feature.completed.presentation.screen.CompletedScreen
...
private fun HomeContent(
    ...
    Column(
        ...
        HomeTopBar(
            titleRes = if (selectedTab == HomeTab.Active) R.string.active_title else R.string.active_completed_tab,
            iconTint = colors.taskList.primaryText,
            onSettingsClick = onSettingsClick,
        )

        if (selectedTab == HomeTab.Active) {
            Spacer(modifier = Modifier.height(FILTER_TOP_SPACING))
            ActiveFilters(colors = colors, selectedCategory = selectedCategory, onCategorySelected = onCategorySelected)
        } else {
            Spacer(modifier = Modifier.height(COMPLETED_LIST_TOP_SPACING))
        }
        ...
        Box(
            ...
            when (selectedTab) {
                HomeTab.Active -> {
                    ActiveScreen(
                        tasks = contentState.activeTasks.filter {
        ...
        PhotoTaskCategory.entries.forEach { category ->
            FilterChip(
                ...
                iconRes = when (category) {
                    PhotoTaskCategory.Parking -> R.drawable.ic_filter_car
                    PhotoTaskCategory.Buy -> R.drawable.ic_filter_buy
...
private fun FilterChip(
    ...
    val contentColor = if (selected) colors.selectedChipContent else colors.taskList.secondaryText

    Surface(
        onClick = onClick,
        shape = RoundedCornerShape(CHIP_CORNER_RADIUS),
        color = if (selected) colors.selectedChipBackground else colors.taskList.cardBackground,
        border = BorderStroke(
            CHIP_BORDER_WIDTH,
            if (selected) colors.selectedChipBorder else colors.taskList.cardBorder,
        ),
    ) {
        Row(
            modifier = Modifier
...
@Composable
@Suppress("UnusedPrivateMember")
private fun HomeActiveTasksLightPreview() {
    PicmorrowTheme(darkTheme = false) {
        HomeScreen(
            darkTheme = false,
            onTakePhotoClick = {},
            onSettingsClick = {},
            contentState = HomeContentState(activeTasks = previewTasks),
        )
    }
}

@Preview(
    name = "Home Active Tasks - Dark",
    showBackground = true,
    showSystemUi = true,
    widthDp = PHONE_PREVIEW_WIDTH,
    heightDp = PHONE_PREVIEW_HEIGHT,
...
@Composable
@Suppress("UnusedPrivateMember")
private fun HomeActiveTasksDarkPreview() {
    PicmorrowTheme(darkTheme = true) {
        HomeScreen(
            darkTheme = true,
            onTakePhotoClick = {},
...
private val COMPLETED_LIST_TOP_SPACING = 16.dp
